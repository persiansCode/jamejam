    ...
    <div class="grid grid-cols-12">
        ...
        <div class="col-span-12 md:col-span-10 pb-14">
            <div class="  flex flex-col md:flex-row  w-full items-center justify-around mt-38 p-[25px] gap-6 ">
                <a class="btn bg-yellow-400 text-center rounded-md p-4  w-[80%] md:w-[20%]" href="{{ route('cms.create-images') }}" > ساخت عکس </a>
                <div
        ...
        <table class="text-white mt-24 text-center bg-slate-700 custom-table rounded-sm w-full">
            <thead>
                <tr>
                    ...
                    <th class="p-3">اسم عکس</th>

                    <th class="p-3">عکس</th>
                    <th class="p-3">اپلود کننده عکس</th>
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-600">
                    ...
                    <td class="p-3">{{ $image->categories->title }}</td>
                    <td class="p-3">{{ $image->title }}</td>

                    <td class="p-3">
                        <div class="flex justify-center">
                            <img src="{{ asset($image->image['indexArray']['small'] ?? 'images/default.jpg') }}"
                                 class="rounded-md w-[50px] h-[50px] object-cover" alt="">
                        </div>
                    </td>
                    <td class="p-3">{{ $image->users->name }}</td>

                    <td class="p-3">
                         <x-cms.check-box :itemId="$image->id" :checked="$image->status"
                                            componentName="show-images-cms-page" />
                    </td>
                    <td class="p-3">
                        <div class="flex justify-center">
                            <button class="bg-red-500 rounded-md p-2 flex justify-center items-center gap-1"
                                    wire:click="deleteItem({{ $image->id }})">
            ...
            <div class="">
                {{ $images->withQueryString()->links('pagination::tailwind') }}
            </div>
        </div>
    </div>
    @push('script')
